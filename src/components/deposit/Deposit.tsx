...
export default function Deposit() {
	...
	const deposit = () => {
		...
		(window as any).tronWeb.contract(getABI('Vault'), getAddress('Vault')).methods.deposit(token.id, _amount).send({})
		.then((res: any) => {
			setLoading(false);
			setSuccess(true);
			setHash(res);
	...
	const approve = () => {
		...
		let token = tokens[selectedToken];
		let _amount = Big(amount).times(10**token.decimals).toString();
		(window as any).tronWeb.contract(getABI('ERC20'), tokens[selectedToken].id).methods.approve(getAddress('Vault'), ethers.constants.MaxUint256.toString()).send({
	...
	return (
		<>
			...
			{ needsApproval() ? 
			<Button width={'100%'} mt={2} onClick={approve} isLoading={loading} loadingText='Approving your token...'>
				Approve zexe to use your token
			</Button>
			:
			<Button width={'100%'} mt={2} disabled={Number(amount) == 0 || amountExceedsBalance()} onClick={deposit} isLoading={loading} loadingText='Confirm in your wallet'>
				{Number(amount) == 0 ? 'Enter Amount' : amountExceedsBalance() ? 'Insufficient Balance' : 'Deposit'}
			</Button>
			}
			<Box>
