...
export default function LendModal({ market, token }) {
  ...
  const approve = async () => {
    ...
    send(cToken, 'approve', [market?.id, ethers.constants.MaxUint256], chain)
    .then((res: any) => {
      console.log(res)
      setLoading(false)
    })
  ...
  return (
		<>
            ...
            <Box>
						...
						<InputGroup>
							<InputLeftAddon bgColor={'transparent'} borderColor='whiteAlpha.300' borderRadius={0}>
								<Image
              ...
              <InputRightAddon bgColor={'transparent'} borderRadius={0} borderColor='whiteAlpha.300'>
              <Button
									variant={"ghost"}
									h="1.75rem"
									size="sm"
            ...
            <Button width={"100%"} bgColor="primary" disabled={amountExceedsBalance() || loading} isLoading={loading} loadingText="Sign the transaction in your wallet" onClick={deposit}>
							{amountExceedsBalance() ? 'Insufficient Balance' : 'Submit'}
						</Button>
            ...
            :<Box>
              ...
              <Image
								...
								/>
                <Box>
                  <Text>To Deposit {token?.name} tokens to zexe, you need to enable it first  </Text>
                </Box>
              </Flex>

            <Button width={"100%"} onClick={approve} isLoading={loading} loadingText='Approving...'>
              Approve {token?.name}
            </Button>
            </Box>
            }
